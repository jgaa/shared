...
The normal desktop test flow is now single-process per instance: `sharedgui` starts the background services in-process.

`shareddaemon` still exists as an optional headless frontend when built with `-DSHARED_BUILD_HEADLESS=ON`, but the primary desktop test path should use `sharedgui` only.

Single-host manual test for one trusted agent and one joining client:

```sh

# Terminal 1: trusted agent desktop instance
rm -f /tmp/shared*.log
...
# Terminal 2: joining client desktop instance
...
7. Confirm `/tmp/shared-a-gui.log` contains:
   `trusted-agent enrollment server listening on 0.0.0.0 47123` or your chosen host and port.
8. Confirm the same log also shows the peer service listening on `127.0.0.1 47124`.

Joining client:

1. Leave the trusted agent running in Terminal 1.
2. Start Terminal 2 for the joining client.
...
15. Confirm `/tmp/shared-b-gui.log` shows the local peer service listening on `127.0.0.1 47125`.
16. Confirm both log files capture the activity without unexpected TLS or OpenSSL errors:
    `/tmp/shared-a-gui.log`
    `/tmp/shared-b-gui.log`

Optional headless build check:

1. Configure with `-DSHARED_BUILD_GUI=OFF` when you want a non-GUI build.
2. Confirm `shareddaemon` still builds and starts as the headless frontend.
3. Use the same `XDG_*` and `SHARED_PEERSERVICE_*` environment setup as above.
